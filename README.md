...
![download latest](https://img.shields.io/github/downloads/devopscorner/devopscorner-container/2.5/total)
...
- Create Tags Image
  ...
  - With Script:
    ...
    ```
    cd ${PATH_DOCKER}/cicd-alpine && docker tag $BASE_IMAGE:$TAG alpine:${ALPINE_VERSION}
    cd ${PATH_DOCKER}/cicd-ubuntu && docker tag $BASE_IMAGE:$TAG ubuntu:${UBUNTU_VERSION}
    cd ${PATH_DOCKER}/cicd-codebuild && docker tag $BASE_IMAGE:$TAG codebuild:${CODEBUILD_VERSION}

    -- or --

    cd ${PATH_COMPOSE} && ./ecr-tag.sh ARGS=YOUR_AWS_ACCOUNT alpine ${ALPINE_VERSION} CI_PATH=devopscorner/cicd
    cd ${PATH_COMPOSE} && ./ecr-tag.sh ARGS=YOUR_AWS_ACCOUNT ubuntu ${UBUNTU_VERSION} CI_PATH=devopscorner/cicd
    cd ${PATH_COMPOSE} && ./ecr-tag.sh ARGS=YOUR_AWS_ACCOUNT codebuild ${CODEBUILD_VERSION} CI_PATH=devopscorner/cicd
...
- With Script:
    ...
    ```
    cd ${PATH_COMPOSE} && ./ecr-push.sh ARGS=YOUR_AWS_ACCOUNT alpine CI_PATH="devopscorner/cicd"
    cd ${PATH_COMPOSE} && ./ecr-push.sh ARGS=YOUR_AWS_ACCOUNT ubuntu CI_PATH="devopscorner/cicd"
    cd ${PATH_COMPOSE} && ./ecr-push.sh ARGS=YOUR_AWS_ACCOUNT codebuild CI_PATH="devopscorner/cicd"
...
- Push Image to **DockerHub** with Tags
  ...
  - With Script:
    ...
    ```
    cd ${PATH_COMPOSE} && ./dockerhub-push.sh alpine CI_PATH="devopscorner/cicd"
    cd ${PATH_COMPOSE} && ./dockerhub-push.sh ubuntu CI_PATH="devopscorner/cicd"
    cd ${PATH_COMPOSE} && ./dockerhub-push.sh codebuild CI_PATH="devopscorner/cicd"
